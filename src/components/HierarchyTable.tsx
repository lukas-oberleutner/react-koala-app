...
const HierarchyTable: React.FC<HierarchyTableProps> = ({ data, level = 0 }) => {
  ...
  const hasChildren = (record: IRecord) =>
  ...
  // Pro zjištění struktury dat pro ladění TODO: DELETE
  ...
  return (
    <table className="border-collapse w-full">
      ...
      <tbody>
        {data.map((record) => {
          // Zkontrolujeme, zda record a jeho data existují TODO: DELETE
          console.log(`Record ${record.data.ID}:`, record);
          ...
          return (
            <React.Fragment key={record.data.ID}>
                ...
                <td className="p-2 text-center">
                  {hasChildren(record) && (
                    <Button
                      ...
                      ariaLabel={
                        expanded[record.data.ID] ? 'Collapse row' : 'Expand row'
                      }
              ...
              {expanded[record.data.ID] &&
                record.children &&
                Object.values(record.children).length > 0 && (
                  <tr>
                    <td colSpan={keys.length + 1} className={'p-4 text-center'}>
                      <HierarchyTable
                        data={Object.values(record.children)[0]?.records || []}
                        level={level + 1}
                      />
                    </td>
                  </tr>
